          ...
          <div className="relative flex h-full w-full max-w-md flex-col bg-white shadow-2xl dark:bg-zinc-900">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-6">
              ...
              <dl className="space-y-3 text-sm">
                ...
                <Row label="Hostel" value={selected.hostel || "—"} />
                <Row label="Orders" value={String(selected.orderCount ?? 0)} />
                <Row label="Joined" value={formatDateLong(selected.createdAt)} />
...
function Avatar({ name, photo, size = "md" }) {
  const sizeCls = size === "lg" ? "h-16 w-16 text-xl" : "h-9 w-9 text-xs";
  if (photo) {
    return (
      <img
        src={photo}
        alt=""
        className={`${sizeCls} shrink-0 rounded-full object-cover ring-2 ring-white dark:ring-zinc-800`}
      />
    );
  }
  ...
  return (
    <div
      className={`${sizeCls} flex shrink-0 items-center justify-center rounded-full bg-[#0077C8]/15 font-bold text-[#0077C8] dark:bg-sky-500/20 dark:text-sky-400`}
    >
      {initial}
    </div>
  );
