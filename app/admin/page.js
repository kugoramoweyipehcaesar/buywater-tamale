          ...
          <>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                <div className="mb-2 text-[11px] font-semibold text-slate-600">Live orders</div>
                <p className="text-4xl font-black text-black">{live.length}</p>
              </div>
              <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                <div className="mb-2 text-[11px] font-semibold text-slate-600">All orders</div>
                <p className="text-4xl font-black text-black">{orders.length}</p>
              </div>
              <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                <div className="mb-2 text-[11px] font-semibold text-slate-600">Revenue</div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    <div key={o.id} className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500">
                          {o.gallons ? `${o.gallons}× unit` : "—"}
                          {o.customerName ? ` · ${o.customerName}` : ""}
                        </p>
                      </div>
                      <span
                        className={`shrink-0 rounded-md px-2.5 py-1 text-[11px] font-bold ${statusBadge(o.status)}`}
                      >
              ...
              <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                ...
                <div className="space-y-2.5">
                  ...
                  <Link
                    href="/admin/users"
                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white py-3 text-sm font-medium text-black hover:bg-slate-50"
                  >
              ...
              {filteredOrders.map((o) => (
                <div key={o.id} className="rounded-2xl border bg-white p-4 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <span className={`rounded-md px-2 py-0.5 text-xs font-bold ${statusBadge(o.status)}`}>
                      {statusLabel(o.status)}
                    </span>
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <select
                      ...
                      className="rounded-lg border px-2 py-1.5 text-xs text-black"
                    >
                      {STATUS.map((s) => (
                    ...
                    <button
                      ...
                      className="rounded-lg border border-red-100 px-2 py-1.5 text-xs text-red-600"
                    >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 rounded-2xl border bg-white p-5 shadow-sm">
              <Toggle
                ...
                checked={!!form.serviceActive}
                onChange={(v) => setForm((f) => ({ ...f, serviceActive: v }))}
              />
              <Field
                label="Hero title"
              ...
              <Field
                ...
                onChange={(v) => setForm((f) => ({ ...f, operatingHours: v }))}
              />
              <Field
                label="Service area"
              ...
              <div className="grid grid-cols-2 gap-3">
                <Num
                  ...
                  onChange={(v) => setForm((f) => ({ ...f, pricePerGallon: v }))}
                />
                <Num
                  label="Sub price"
                  value={form.subscriptionPrice}
                  onChange={(v) => setForm((f) => ({ ...f, subscriptionPrice: v }))}
                />
                <Num
                  label="Sub gallons"
                  value={form.subscriptionGallons}
                  onChange={(v) => setForm((f) => ({ ...f, subscriptionGallons: v }))}
                />
                <Num
                  label="Delivery min"
                  value={form.deliveryTimeMin}
                  onChange={(v) => setForm((f) => ({ ...f, deliveryTimeMin: v }))}
                />
                <Num
                  label="Delivery max"
                  value={form.deliveryTimeMax}
                  onChange={(v) => setForm((f) => ({ ...f, deliveryTimeMax: v }))}
                />
              </div>
              <Field
                label="Admin phone"
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={addHostel}
                disabled={busy}
                className="rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white disabled:opacity-60"
              >
                Add
              </button>
            </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPromo((p) => ({ ...p, rewardValue: Number(e.target.value) }))
                    }
                    className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium text-black"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPromo((p) => ({ ...p, maxUses: Number(e.target.value) }))
                    }
                    className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-medium text-black"
                  />
                </div>
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-sm text-black">
                      <span className="font-semibold">{p.rewardValue}</span> free gallon
                      {Number(p.rewardValue) === 1 ? "" : "s"} · Used by{" "}
                      <span className="font-semibold">{p.timesUsed ?? 0}</span> /{" "}
                  ...
                  <button
                    ...
                    disabled={busy}
                    className="shrink-0 rounded-lg border border-red-100 px-2.5 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50 disabled:opacity-50"
                  >
