...
import {
  Droplets, Loader2, Minus, Plus, Banknote, Smartphone, BadgePercent, ArrowLeft, AlertTriangle,
} from "lucide-react";
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([me, set, hos]) => {
        ...
        else if (s.momoEnabled === false && s.cashEnabled !== false) setPaymentMethod("cash_on_delivery");
      })
      .catch(() => {})
    ...
    <div className="min-h-screen bg-[#EEF6FC] pb-12">
      <div className="mx-auto max-w-lg px-4 py-6">
        ...
        <form onSubmit={placeOrder} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          <button type="button" disabled={maintenanceOn} onClick={() => setIsSubscription(!isSubscription)} className={`mb-5 flex w-full items-center gap-3 rounded-xl border px-4 py-3 text-left ${isSubscription ? "border-[#0077C8] bg-[#0077C8]/5" : "border-slate-200 bg-slate-50"}`}>
