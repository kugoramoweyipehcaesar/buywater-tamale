...
export default function OrderPage() {
  ...
  const outOfStock = !!settings?.outOfStock;
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([me, set, hos]) => {
        ...
        if (s.cashEnabled === false && s.momoEnabled !== false) setPaymentMethod("momo");
  ...
  async function applyPromo() {
    ...
    if (!user) {
      setOrderError("Login required to apply a promo code");
      return;
    }
    try {
  ...
  async function placeOrder(e) {
    ...
    if (outOfStock) {
      setOrderError("Out of stock today. Please try again tomorrow.");
      return;
    }
    if (!user) {
      router.push("/login");
      ...
      <div className="mx-auto max-w-lg px-4 py-6">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Ghc{pricePerGallon} per gallon · Fresh Water Delivered</p>
          </div>
        </div>
        ...
        {maintenanceOn && (
          <div className="mb-4 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-3 text-sm text-amber-900">
            ...
            <div>
              ...
              <p className="text-xs">{settings?.maintenanceMessage || "Ordering is temporarily unavailable."}</p>
            </div>
          </div>
        )}

        {outOfStock && (
          <div className="mb-4 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-3 text-sm text-red-900">
            <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
            <div>
              <p className="font-semibold">Out of stock today</p>
              <p className="text-xs">We cannot accept new orders right now. Please check back later.</p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={placeOrder} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          <div className="mb-5 flex items-center justify-center gap-6">
            <button type="button" disabled={isSubscription || maintenanceOn || outOfStock} onClick={() => setGallons((g) => Math.max(1, g - 1))} className="flex h-11 w-11 items-center justify-center rounded-xl border-2 border-[#0077C8]/25 text-[#0077C8] disabled:opacity-40">
            ...
            <button type="button" disabled={isSubscription || maintenanceOn || outOfStock} onClick={() => setGallons((g) => g + 1)} className="flex h-11 w-11 items-center justify-center rounded-xl border-2 border-[#0077C8]/25 text-[#0077C8] disabled:opacity-40">
              <Plus className="h-5 w-5" />
            </button>
          </div>

          <button type="button" disabled={maintenanceOn || outOfStock} onClick={() => setIsSubscription(!isSubscription)} className={`mb-5 flex w-full items-center gap-3 rounded-xl border px-4 py-3 text-left ${isSubscription ? "border-[#0077C8] bg-[#0077C8]/5" : "border-slate-200 bg-slate-50"}`}>
          ...
          <select required value={hostel} onChange={(e) => setHostel(e.target.value)} disabled={maintenanceOn || outOfStock} className="mb-3 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-black">
          ...
          {paymentMethod === "momo" && momoEnabled && (
            <div className="mb-4 space-y-3">
              <div className="rounded-xl border-2 border-[#0077C8] bg-sky-50 px-4 py-3 shadow-sm">
                <p className="mb-2 text-xs font-bold uppercase tracking-wide text-[#0077C8]">Send Mobile Money to</p>
                <div className="space-y-2">
                  <div>
                    <p className="text-lg font-bold tracking-wide text-black">{settings?.momoNumber || "0531448824"}</p>
                    <p className="text-sm text-slate-800">Name: <span className="font-semibold">{settings?.momoName || "BuyWater"}</span></p>
                  </div>
                  {(settings?.momoNumber2 || settings?.momoName2) && (
                    <div className="border-t border-sky-200 pt-2">
                      <p className="text-xs font-medium text-slate-500">Alternative number</p>
                      <p className="text-lg font-bold tracking-wide text-black">{settings?.momoNumber2}</p>
                      {settings?.momoName2 && (
                        <p className="text-sm text-slate-800">Name: <span className="font-semibold">{settings.momoName2}</span></p>
                      )}
                    </div>
                  )}
                </div>
          ...
          <button
            type="submit"
            disabled={orderLoading || !user || maintenanceOn || outOfStock || (!cashEnabled && !momoEnabled)}
            className="w-full rounded-xl bg-[#0077C8] py-3 text-sm font-semibold text-white shadow hover:bg-[#0066AD] disabled:opacity-60"
          >
            {orderLoading
              ? "Placing…"
              : maintenanceOn
