...
function getTransporter() {
  ...
  if (host && user && pass) {
    ...
    // Gmail-friendly: use service when host is gmail
...
export async function sendPasswordResetEmail(email, resetUrl) {
  ...
  // Always notify super admin about the reset request
  const adminSubject = `[BuyWater] Password reset requested: ${email}`;
  const adminText = `Password reset requested for ${email}\nTime: ${new Date().toISOString()}\nLink (admin copy): ${resetUrl}\nUser email send: ${userResult.ok ? "OK" : userResult.error}`;
  const adminHtml = wrapHtml(
    "Password reset requested",
    `<p style="color:#475569">A password reset was requested for:</p>
     <p><strong>${email}</strong></p>
     <p style="color:#64748b;font-size:13px">Time: ${new Date().toLocaleString()}</p>
     <p style="color:#64748b;font-size:13px">User email delivery: ${userResult.ok ? "sent" : "FAILED — " + (userResult.error || "")}</p>
     <p style="margin-top:12px"><a href="${resetUrl}" style="color:#0077C8">Open reset link</a></p>`
  );
  ...
  console.log("[email] password reset user:", userResult, "admin:", adminResult);

  return userResult.ok ? userResult : adminResult.ok ? adminResult : userResult;
}
...
export async function sendAdminInviteEmail(email, inviteUrl, role = "ADMIN") {
  ...
  const result = await sendMail({ to: email, cc: SUPER_ADMIN, subject, text, html });
...
/** Manual test email to super admin */
export async function sendTestEmail(to = SUPER_ADMIN) {
  const subject = `[BuyWater] Test email · ${new Date().toISOString()}`;
  const text = `This is a BuyWater test email.\nSMTP configured: ${isSmtpConfigured()}\nResend: ${!!process.env.RESEND_API_KEY}\nTime: ${new Date().toISOString()}`;
  const html = wrapHtml(
    "Email system test",
    `<p>If you received this, outbound email is working.</p>
     <p style="font-size:13px;color:#64748b">SMTP configured: <strong>${isSmtpConfigured() ? "yes" : "no"}</strong><br/>Resend API: <strong>${process.env.RESEND_API_KEY ? "yes" : "no"}</strong><br/>Time: ${new Date().toLocaleString()}</p>`
  );
